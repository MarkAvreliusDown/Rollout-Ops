# Исправление миграции 0016: там ошибочно решили, что вкладок Банк-эквайер/Подрядчик 1/
# Подрядчик 2/Оборудование ещё нет, и создали для них новые пустые компании — хотя
# пользователь уже завёл их сам с реальными данными (под случайными ключами c_...). Эта
# миграция удаляет пустые дубли и вместо этого помечает автоподстановкой уже существующие
# настоящие вкладки.
...
def fix_duplicates(apps, schema_editor):
    ...
    apply_tags("Банк-эквайер", SBER_TAGS)
